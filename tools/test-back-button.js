  ...
  try {
    ...
    if (checkoutVisible) {
      ...
      const backButtonExists = await page.evaluate(() => {
        ...
        const backLinks = Array.from(document.querySelectorAll('a')).filter(
          (link) =>
      ...
      if (backButtonExists.buttonExists || backButtonExists.linksFound > 0) {
        ...
        const finalState = await page.evaluate(() => ({
          ...
          checkoutVisible: document.getElementById('checkout-view')
            ? window.getComputedStyle(document.getElementById('checkout-view')).position
            : 'not found',
        }));
