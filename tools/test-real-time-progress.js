...
(async () => {
  ...
  try {
    ...
    if (progressUpdates.length >= 2) {
      console.log(
        `✅ SUCCESS: Real-time progress tracking working! Captured ${progressUpdates.length} updates`
      );
      ...
      console.log(
        `   End:   ${progressUpdates[progressUpdates.length - 1].progress}% at ${progressUpdates[progressUpdates.length - 1].time}ms`
      );
      ...
      if (intermediateUpdates.length > 0) {
        ...
        console.log(
          '⚠️  WARNING: No intermediate progress detected (might still be 0% -> 100% jump)'
        );
      }
    } else {
