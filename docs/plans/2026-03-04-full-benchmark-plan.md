# Full Benchmark Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create `testing/full_benchmark.py` — a standalone 2-phase benchmark that finds the best prompt variant + model for authentic character voice and ideological calibration.

**Architecture:** Standalone script (no app imports), calls Ollama `/api/chat` directly via httpx. Phase 1 fixes the model and varies 4 prompt structures. Phase 2 fixes the best prompt and compares 4 models. LLM-as-judge (dolphin3) scores voice authenticity (0-3).

**Tech Stack:** Python 3.11+, httpx (async), asyncio, argparse, json, re — all already in venv.

---

## Key constants before coding

```
OLLAMA_URL  = http://192.168.2.69:11434
JUDGE_MODEL = dolphin3:latest
PHASE1_MODEL = mistral-small3.1:24b
PHASE2_MODELS = [mistral-small3.1:24b, type32/eva-qwen-2.5-14b, muse-12b:latest, LESSTHANSUPER/MAGNUM_V4-Mistral_Small:12b_Q5_K_M]

Films (with tmdb_id):
  Barbie (2023)        → tmdb_id: 346698
  John Wick (2014)     → tmdb_id: 245891
  Idiocracy (2006)     → tmdb_id: 7299
  El Padrino (1972)    → tmdb_id: 238
```

---

## Task 1: Scaffold the file + constants

**Files:**
- Create: `testing/full_benchmark.py`
...
**Step 3: Add EXPECTED_RATINGS dict**
...
python3 -c "import testing.full_benchmark" 2>&1 || python3 testing/full_benchmark.py --help
```

---

## Task 2: Character souls

**Files:**
- Modify: `testing/full_benchmark.py`

Define `CHARACTERS: dict[str, dict]` with all 6 characters. Each entry has:
- `name`, `emoji`, `description` (identity block)
- `loves`, `hates`, `red_flags`, `avoid` (JSON strings for rubric)
- `anecdotes` (list[str] — 3-4 items, used in V2/V3)
- `catchphrases` (list[str] — used in V2/V3)

**Step 1: Add the 6 character definitions**
...
**Step 2: Verify characters load without errors**

```bash
python3 -c "
import sys; sys.path.insert(0, '.')
exec(open('testing/full_benchmark.py').read().split('# ── PROMPT')[0])
print('Characters OK:', list(CHARACTERS.keys()))
print('Movies OK:', [m['title'] for m in MOVIES])
"
```

Expected output:
```
Characters OK: ['Marco Aurelio', 'Rosario Costras', 'Adolf Histeric', 'Charlie Sheen', 'Antonio Recio', 'Beavis']
Movies OK: ['Barbie', 'John Wick', 'Idiocracy', 'El Padrino']
```

---

## Task 3: Prompt variants V0–V3

**Files:**
- Modify: `testing/full_benchmark.py`

**Step 1: Add SYSTEM_BLOCK (same for all variants — English, output in Spanish)**
...
**Step 2: Add `build_messages(variant, character, movie)` function**
...
**Step 3: Smoke test prompt building**

```python
python3 -c "
import sys; sys.path.insert(0, '.')
# Quick load test
import testing.full_benchmark as b
msgs = b.build_messages(b.VARIANTS[3], b.CHARACTERS['Beavis'], b.MOVIES[0])
print('V3 Beavis×Barbie user block preview:')
print(msgs[1]['content'][:500])
print('...')
print('OK — messages built successfully')
"
```

---

## Task 4: Ollama call + LLM-as-judge

**Files:**
- Modify: `testing/full_benchmark.py`

**Step 1: Add `call_ollama(url, model, messages, think, timeout)` async function**
...
**Step 2: Add `call_judge(url, character_name, critic_text)` async function**
...
**Step 3: Smoke test — verify Ollama is reachable**
...
async def ping():
    async with httpx.AsyncClient(timeout=10) as c:
        r = await c.get('http://192.168.2.69:11434/api/tags')
        models = [m['name'] for m in r.json().get('models', [])]
        print('Ollama reachable. Models:', models[:5])
asyncio.run(ping())
"
```
...
**Step 1: Add `extract_rating`, `score_result` functions**
...
## Task 6: run_one + orchestrators

**Files:**
- Modify: `testing/full_benchmark.py`

**Step 1: Add `run_one` coroutine**
...
**Step 2: Add `run_phase1` and `run_phase2` orchestrators**
...
async def run_phase(
    label: str,
    combos: list[tuple],   # (variant, model, character, movie)
    ollama_url: str,
    use_judge: bool,
) -> list[dict]:
    ...
    for i, (variant, model, character, movie) in enumerate(combos, 1):
        ...
        print(f"  {tag} {variant['id']} | {model[:28]:<28} | {character['name']:<16} × {movie['title']}", end=" ... ", flush=True)
        result = await run_one(variant, model, character, movie, ollama_url, use_judge)
        results.append(result)
        if result["error"]:
            print(f"❌ {result['error'][:60]}")
        else:
            cal = "✅" if result["calibration_ok"] else ("❌" if result["calibration_ok"] is False else "⚠️")
...
## Task 7: Pick winner + Markdown report

**Files:**
- Modify: `testing/full_benchmark.py`

**Step 1: Add `pick_winner` function**
...
**Step 2: Add `generate_report` function**

The report should contain:
1. Metadata header (date, models, config)
2. Phase 1 table: variant × avg_voice_score / calibration% / format%
3. Winner announcement
4. Phase 2 table: model × avg_voice_score / calibration% / speed
5. Per-character best combo table
6. Raw critic samples (3-5 best critics, full text)

Implement as `generate_report(phase1_results, phase2_results, winner_variant, meta) -> str`.

Key aggregate helper:
```python
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--quick",         action="store_true",
                   help="Quick smoke test: 1 char × 1 film × V0 × primary model")
    return p.parse_args()
...
**Step 1: Run quick mode to validate end-to-end**
...
Expected: 2 critics generated (Beavis × Barbie, V0 and V3), voice scores appear, JSON + MD saved.

**Step 2: Check output files**

```bash
ls -lh docs/benchmark-results/*.json docs/benchmark-results/*.md | tail -4
```
...
git commit -m "feat: add full benchmark — prompt×model optimization with LLM-as-judge voice scoring

2-phase: Phase 1 (prompt variants V0-V3) + Phase 2 (4 models head-to-head)
6 characters: Marco Aurelio, Rosario Costras, Adolf Histeric, Charlie Sheen, Antonio Recio, Beavis
4 films: Barbie, John Wick, Idiocracy, El Padrino
LLM-as-judge: dolphin3 scores voice authenticity 0-3
...
**Step 1: Run Phase 1 only first to validate all 96 calls**

```bash
source venv/bin/activate
python3 testing/full_benchmark.py --phase 1 2>&1 | tee docs/benchmark-results/phase1-run.log
```

**Step 2: If Phase 1 looks good, run Phase 2 with winner**

```bash
python3 testing/full_benchmark.py --phase 2 2>&1 | tee docs/benchmark-results/phase2-run.log
```

Or run both at once (takes 40-60 min):

```bash
python3 testing/full_benchmark.py 2>&1 | tee docs/benchmark-results/full-run.log
```
...
git commit -m "results: full benchmark run — prompt×model findings
